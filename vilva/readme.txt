=== Vilva ===

Contributors: blossomthemes
Requires at least: 6.2
Requires PHP: 7.4
Tested up to: 6.9
Stable tag: 1.2.9
License: GNU General Public License v3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html
Tags: blog, one-column, two-columns, right-sidebar, left-sidebar, footer-widgets, custom-background, custom-header, custom-menu, custom-logo, featured-images, threaded-comments, full-width-template, rtl-language-support, translation-ready, theme-options, e-commerce, news

== Description ==
Vilva is an easy to use and mobile friendly multi-purpose blog theme. Whether you want to share your expert fashion tips, your favorite recipes, or create a lifestyle, travel, food, beauty and makeup, coaching, or any other themed blog, Vilva is for you. With Vilva, you can change the look and feel (color and font) of your website with a click. You can flaunt your Instagram posts and increase your followers. Similarly, you can also build an email list and stay in touch with your customers. Vilva is SEO-friendly, speed optimized, translation ready and WooCommerce and RTL compatible. Check demo at https://blossomthemes.com/theme-demo/?theme=vilva, read the documentation at https://docs.blossomthemes.com/docs/vilva/, and get support at https://blossomthemes.com/support-ticket/. 

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload Theme and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Copyright ==

Vilva WordPress Theme, Copyright 2019 blossomthemes.com
Vilva is distributed under the terms of the GNU GPL

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

Vilva bundles the following third-party resources:

Owl-carousel, Copyright 2013-2017 David Deutsch 
License : MIT
Source  : https://owlcarousel2.github.io/OwlCarousel2

Owl Carousel v2 Accessibility Layer, Copyright 2016 Geoffrey Roberts
License : MIT
Source  : https://github.com/rtrvrtg/owlcarousel2-a11ylayer

animate.css, Copyright (c) 2017 Daniel Eden
License: MIT
Source: https://github.com/daneden/animate.css

normalize.css, Copyright 2012-2016 Nicolas Gallagher and Jonathan Neal
License: MIT
Source: https://necolas.github.io/normalize.css/

Font Awesome icons, Copyright Dave Gandy
Icons License: CC BY 4.0 
Fonts License: SIL OFL 1.1
Source: http://fontawesome.io/

Selectize, Copyright � 2013�2016 Brian Reavis & Contributors
License: Apache License, Version 2.0
Source: https://github.com/selectize/selectize.js

Sifter, Copyright � 2013�2015 Brian Reavis & Contributors
License: Apache License, Version 2.0
Source: https://github.com/brianreavis/sifter.js/

Microplugin, Copyright � 2013 Brian Reavis & Contributors
License: Apache License, Version 2.0
Source: https://github.com/brianreavis/microplugin.js

TGMPA, Copyright (c) 2011, Thomas Griffin
License: GPL-2.0+
Source: http://tgmpluginactivation.com/

Vilva Theme incorporates code from Kirki, Copyright (c) 2017 Aristeides Stathopoulos
License: https://github.com/aristath/kirki/blob/develop/LICENSE
Source: https://github.com/aristath/kirki

Vilva local fonts feature is based on Webfonts Loader by WordPress Themes Team (WPTT). (https://github.com/WPTT/webfont-loader)
License: MIT
Source: https://github.com/WPTT/webfont-loader/blob/master/LICENSE

Some codes are taken from https://blog.josemcastaneda.com/2016/08/08/copy-paste-hurting-theme/

Some codes are taken from Twenty Twenty theme - https://wordpress.org/themes/twentytwenty/

Images used in screenshots.
Source:
https://stocksnap.io/photo/S6RLOBPAOZ
https://stocksnap.io/photo/TXGIMQSM8D
https://stocksnap.io/photo/KLSMMEWGNA
https://stocksnap.io/photo/GAXFQX8BXD
https://pxhere.com/en/photo/1451201
https://stocksnap.io/photo/OS9ZOMONVP
License: CC0, https://stocksnap.io/license, https://pxhere.com/en/license

All other images including icon image used in screenshot and images used in customizer settings are self created and are under the GPL.

== Changelog ==
    
    1.2.9 - April 06, 2026
        * Fix: Updated PHP to 7.4
        * Update: Google Fonts Library has been updated.

     1.2.8 - April 21, 2025
        * Fix: PHP warning issue while viewing WooCommerce product tag page has been fixed.
        * Fix: Translation loaded early issue has been fixed.
        * Update: Google Fonts Library has been updated.

    1.2.7 - December 02, 2024
        * Update: Compatibility with new banner layouts of Delicious Recipes Pro version 2.2.3.
        * Update: Google Fonts Library has been updated.

    1.2.6 - June 06, 2024
        * Update: New dashboard has been implemented for better user experience.
        * Update: Google Fonts Library has been updated.
        
    1.2.5 - May 17, 2024
        * Fix: Minor refinements have been done in the dashboard links.
        * Update: Google Fonts Library has been updated.

    1.2.4 - February 06, 2024
        * Update: Ajax requests issue has been fixed.
        
    1.2.3 - February 01, 2023
        * Update: Added nonce to the custom admin notice to avoid CSRF vulnerability.
        * Update: Google Fonts Library has been updated.
        
    1.2.2 - November 1, 2023
        * Update: Google Fonts Library has been updated.
        * Update: Now users can add Instagram shortcode from any plugins of their choice

    1.2.1 - July 5, 2023
        * Fix: Warning shown during plugin installation through TGMPA for PHP 8 users has been fixed.
        * Fix: Added underline in widget text link. 
        * Fix: Minor refinements have been done to fix accessibilty issue.
        * Update: Google Fonts library has been updated.
        * Update: Relevant tags for the theme have been added.

    1.2.0 - September 09, 2022
        * Fix: breadcrumb issue fixed

    1.1.9 - July 15, 2022
        * Fix: issue of images and site-logo in single post schema

    1.1.8 - July 07, 2022
        * Added: feature to load google font locally 

    1.1.7 - May 11, 2022
        * Fixed: Pagination design issue
        * Updated: Added FontAwesome icon for TikTok
        * Updated: Added compatibility for autoload feature of Delicious Recipe Pro plugin
        * Updated: Google Fonts library
        * Fixed: RTL menu design issue

    1.1.6 - February 22, 2021
        * Recommended : AffiliateX plugin
        * Fixed : Footer partial refresh 

    1.1.5 - Oct 8, 2021
        * Fixed : css refinements for child theme

    1.1.4 - July 21, 2021
        * Removed: block widget for WordPress 5.8 compatibility
        * Fixed: design issue of single posts sticky meta

    1.1.3 - June 14, 2021
        * Fixed: CTA widget title color issue
        * Fixed: Newsletter issue 

    1.1.2 - December 25, 2020
        * Fixed: customizer load issue

    1.1.1 - November 26, 2020
        * Fixed: widgets design in sidebar
        * Fixed: content links designs
    
    1.1.0 - October 22, 2020
        * Modified: css refinement for child theme

    1.0.9 - October 20, 2020
        * Modified: made compatible with WP Delicious plugin
    
    1.0.8 - October 06, 2020
        * Modified: made compatible with child theme

    1.0.7 - September 30, 2020
        * Fixed: changed free vs pro image
        * Fixed: translation issue

    1.0.6 - September 22, 2020
        * Fixed: social media settings icon issue
        * Fixed: slider control value not showing issue
        * Modified: theme homepage link

    1.0.5 - June 11, 2020
        * Fixed: author name issue
        * Fixed: naming of recommeneded plugin issue

    1.0.4 - June 05, 2020
        * Fixed: issues listed from org reviewer

    1.0.3 - June 04, 2020
        * Added: increase primary font option
        * Added: option to add repetive post in blog and slider
        * Modified: make gutenberg compatible 
        * Fixed: other design issues
	
	1.0.2 - April 23, 2020
        * Fixed: accessibilty toggle issue

    1.0.1 - March 18, 2020
        * Added: Owl Carousel v2 Accessibility Layer js
        * Fixed: instagram section design issue

    1.0.0 - February 4, 2020
    * Initial release